...
onClickOutside(profileRef, (event) => (isOpenProfile.value = false));
const isOpenProfile = ref(false);
const isDark = useDark();
const toggleTheme = useToggle(isDark);
const isShow = ref<boolean>(false);
const emit = defineEmits<{ (e: "toggleNavbar", value: boolean): void }>();
const btnNavbar = ref();
onClickOutside(btnNavbar, (event) => {
  isShow.value = false;
  emit("toggleNavbar", isShow.value);
});
      ...
      <ul class="flex items-center shrink-0 space-x-6">
        ...
        <li class="relative">
          <button
            ...
            @keydown.esc="isOpenProfile = false"
            ref="profileRef"
            aria-label="Account"
            aria-haspopup="true"
          >
